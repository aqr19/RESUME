...
public class YouTubeMPDBuilder implements MPDBuilder {
    ...
    private void writeMediaTags() {
        ...
        // Separate and prioritize AV1 (av01) formats first for highest efficiency & quality
        java.util.TreeSet<MediaFormat> av1Videos = new java.util.TreeSet<>(new MediaFormatComparator());
        java.util.TreeSet<MediaFormat> avcVideos = new java.util.TreeSet<>(new MediaFormatComparator());
        for (MediaFormat fmt : mMP4Videos) {
            if (fmt.getMimeType() != null && fmt.getMimeType().contains("av01")) {
                av1Videos.add(fmt);
            } else {
                avcVideos.add(fmt);
            }
        }
        if (!av1Videos.isEmpty()) {
            writeMediaTagsForGroup(av1Videos);
        }
        if (!avcVideos.isEmpty()) {
            writeMediaTagsForGroup(avcVideos);
        }
